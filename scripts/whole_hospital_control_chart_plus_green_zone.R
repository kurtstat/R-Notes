# Jagged lines of fullness with specification limits AND green zone
# What was compared with what should've been
...
dataframe_01 <-
  read_xlsx("data/ari_duration_extract_2023_24.xlsx") |> 
  filter(SPECIALTY_CODE == "GM")
...
amr <- mean(abs(diff((dataframe_01a$new_mean_occupied_beds))))
lcl <- overall_mean - (2.66 * amr) 
ucl <- overall_mean + (2.66 * amr)
bed_allocation = 72
green_lcl = bed_allocation - (ucl - lcl)
green_mean = bed_allocation - ((ucl - lcl) / 2)
...
ggplot(data = dataframe_01b) +
  ...
  labs(title = "Average no. of beds occupied per week",
       subtitle = "Anytown Royal Infirmary | General Medicine",
       x = element_blank(),
       y = element_blank())
